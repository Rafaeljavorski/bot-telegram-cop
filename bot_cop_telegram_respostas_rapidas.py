...
RESPOSTAS_RAPIDAS = {
    ...
    "psw_recusado": (
        "❌ PSW recusado.\n\n"
        "Verifique as evidências enviadas e encaminhe novamente conforme o padrão."
    ),
    "rede": (
        "🌐 Chamado encaminhado para a equipe de Rede.\n\n"
        "Aguarde novas orientações."
    ),
    "nap": (
        "📡 Estamos verificando as informações da NAP.\n\n"
        "Aguarde um momento."
    ),
...
def teclado_respostas_rapidas(protocolo):
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton(
                "📍 Aguarde um instante",
                callback_data=f"resp:{protocolo}:aguarde"
            ),
            InlineKeyboardButton(
                "📞 Ligando para o cliente",
                callback_data=f"resp:{protocolo}:ligando"
            ),
        ],
        [
            InlineKeyboardButton(
                "🟢 PSW liberado",
                callback_data=f"resp:{protocolo}:psw_liberado"
            ),
            InlineKeyboardButton(
                "🔴 PSW recusado",
                callback_data=f"resp:{protocolo}:psw_recusado"
            ),
        ],
        [
            InlineKeyboardButton(
                "🌐 Encaminhado para Rede",
                callback_data=f"resp:{protocolo}:rede"
            ),
            InlineKeyboardButton(
                "📡 Verificando NAP",
                callback_data=f"resp:{protocolo}:nap"
            ),
        ],
        [
            InlineKeyboardButton(
                "📸 Reenviar evidências",
                callback_data=f"resp:{protocolo}:evidencias"
            ),
        ],
        [
            InlineKeyboardButton(
                "⬅️ Voltar",
                callback_data=f"respostas_voltar:{protocolo}"
            ),
        ],
    ])


async def mostrar_menu_respostas_rapidas(query, protocolo):
    ticket = buscar_ticket(protocolo)

    if not ticket or ticket.get("status") != "em_atendimento":
        await query.answer("Atendimento não está mais ativo.", show_alert=True)
        return
    ...
    try:
        ...
        logger.warning("Não consegui abrir respostas rápidas de %s: %s", protocolo, e)


async def voltar_menu_controles(query, protocolo):
    ticket = buscar_ticket(protocolo)
    ...
    try:
        ...
        logger.warning("Não consegui voltar aos controles de %s: %s", protocolo, e)


async def enviar_resposta_rapida(query, context, protocolo, chave):
    ticket = buscar_ticket(protocolo)
    ...
    if ticket.get("atendente_id") and ticket["atendente_id"] != user.id and not eh_admin(user.id):
        await query.answer("Você não é o responsável por esse atendimento.", show_alert=True)
        return

    texto = RESPOSTAS_RAPIDAS.get(chave)
    if not texto:
        await query.answer("Resposta rápida inválida.", show_alert=True)
        return
    ...
    try:
        ...
        logger.warning("Não consegui enviar resposta rápida ao técnico em %s: %s", protocolo, e)
        await query.answer("Não consegui enviar ao técnico.", show_alert=True)
        return
    ...
    try:
        await context.bot.send_message(
            ...
            message_thread_id=ticket["message_thread_id"],
            text=(
                f"📤 *Resposta rápida enviada por {user.full_name}*\n\n"
                f"{texto}"
            ),
            parse_mode="Markdown",
        )
    except Exception as e:
        logger.warning("Não consegui registrar resposta rápida no tópico de %s: %s", protocolo, e)

    await query.answer("Resposta enviada.")

    # Mantém o submenu de respostas rápidas como a última mensagem do tópico.
    ticket_atual = buscar_ticket(protocolo)
    mensagem_controle = ticket_atual.get("control_message_id") if ticket_atual else None

    if mensagem_controle:
        try:
            await context.bot.delete_message(
                chat_id=destino_ticket(ticket_atual),
    ...
    try:
        ...
        atualizar_ticket(protocolo, control_message_id=msg_menu.message_id)
    except Exception as e:
        logger.warning("Não consegui manter respostas rápidas no final de %s: %s", protocolo, e)
...
async def botoes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("respostas_menu:"):
        ...
        await mostrar_menu_respostas_rapidas(query, protocolo)
        return

    if data.startswith("respostas_voltar:"):
        protocolo = data.split(":", 1)[1]
        await voltar_menu_controles(query, protocolo)
        return
    ...
    if data.startswith("resp:"):
        ...
        if len(partes) == 3:
            ...
            await enviar_resposta_rapida(query, context, protocolo, chave)
        return
